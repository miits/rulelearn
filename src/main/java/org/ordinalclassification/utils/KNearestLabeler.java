...
public class KNearestLabeler {
    ...
    public LearningExampleType labelExampleWithRareCheck(int exampleIndex, HashMap<Integer, int[][]> neighbourhoods) {
    ...
    public LearningExampleType labelExampleWithRareCheckForMonotonicNeighbourhood(int exampleIndex, HashMap<Integer, int[][]> neighbourhoods, int[][] monotonicNeighbourhood) {
        int[] minClassExamples = monotonicNeighbourhood[0];
        if (minClassExamples.length >= safeLowerLimit) return LearningExampleType.SAFE;
        if (minClassExamples.length >= borderlineLowerLimit) return LearningExampleType.BORDERLINE;
        if (minClassExamples.length == rareLowerLimit) {
            int[] minNeighbourhood = neighbourhoods.get(minClassExamples[0])[0];
